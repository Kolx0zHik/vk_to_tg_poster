...
| видео | если VK отдаёт картинку `image`/`first_frame` — `sendPhoto` с превью, первой строкой `Видео: <название-ссылка>`, статистикой, текстом поста и кнопкой; без картинки — текстовое сообщение с HTML-ссылкой |
