...
- ИИ-проверка дублей — совещательная: она только помечает пост `skipped`. При недоступном LLM, пустых
  ...
  - Временный тумблер `LLM_DEBUG_LOG` (`1`/`true`/`yes`/`on`; см. README): на INFO пишет вердикт с причиной
    и факт пустого пула кандидатов по каждому посту; сырой ответ LLM логируется только когда он не разбирается
    как JSON (до 500 символов), иначе в логе одна строка на пост; без переменной — тишина, как раньше.
