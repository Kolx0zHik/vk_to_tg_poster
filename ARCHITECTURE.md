...
| >1 фото | `sendMediaGroup` без caption, затем текст отдельным сообщением (обрезка до 4096) |
...
Длинный текст не разбивается на несколько сообщений, а обрезается как caption: `_truncate_text` режет тело
до `MESSAGE_LIMIT=4096` (для caption — до 1024) по границам абзаца → строки → предложения → слова и
добавляет пометку «Продолжение текста читайте в источнике». Для HTML-вариантов (`parse_mode=HTML` в
caption и в сообщениях с разметкой) смещение дополнительно отводится назад `_safe_offset`, чтобы не
разорвать HTML-сущность или тег; в обычном тексте (`PLAIN_CONTINUATION`) такой отвод не нужен. При наличии
статистики видео её место резервируется заранее, кнопка «Открыть пост в VK» ставится на сообщение.
...
- Ограничения Telegram: текст длиннее 4096 обрезается с пометкой о продолжении, caption — до 1024;
