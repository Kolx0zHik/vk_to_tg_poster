...
| >1 фото | `sendMediaGroup` без caption, затем текст отдельными сообщениями (разбивка по `MESSAGE_LIMIT=4096`) |
...
| текст | отдельными сообщениями, если ещё не израсходован |

Длинный текст режется в `_split_text`: готовое тело сообщения делится на части ≤ `MESSAGE_LIMIT=4096` по
границам абзаца → строки → предложения → слова, не разрывая HTML-сущности и теги (`_safe_offset`). Кнопка
«Открыть пост в VK» ставится только на последнюю часть. Caption (фото/видео/аудио) разбить нельзя, поэтому
для них работает обрезка `_truncate_html_text` с пометкой «Продолжение текста читайте в источнике»; при
наличии статистики видео её место резервируется заранее.
...
- Ограничения Telegram: текст длиннее 4096 режется на части, caption — до 1024; альбом больше 10 медиа
  по-прежнему не разбивается — см. STATE.
