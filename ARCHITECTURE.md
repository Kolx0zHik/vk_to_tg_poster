...
| >1 фото | `sendMediaGroup` без caption, затем текст отдельным сообщением с кнопкой |
| видео с `.mp4/.mov/.mkv` | `sendVideo` по URL, caption — текст и/или статистика |
| прочее видео | текстовое сообщение со ссылкой на VK-плеер |
| аудио с URL | `sendAudio`; без URL — ссылка |
...
- Ограничения Telegram (4096 символов в тексте, максимум 10 медиа в альбоме) не проверяются — см. STATE.
