...
- ИИ-проверка дублей — совещательная: она только помечает пост `skipped`. При недоступном LLM, пустых
  ...
  - Временный тумблер `LLM_DEBUG_LOG` (`1`/`true`/`yes`/`on`; см. README): на INFO пишет сырой ответ LLM
    (до 500 символов), вердикт с причиной и факт пустого пула кандидатов; без переменной — тишина, как раньше.
