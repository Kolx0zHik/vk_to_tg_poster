...
Секция `llm` в созданном конфиге уже содержит стартовые `base_url`, модель и системный промпт ИИ-проверки —
их можно править из панели («ИИ-проверка»), а саму проверку включить тумблером (для неё нужен `LLM_API_KEY`
в `.env`).
