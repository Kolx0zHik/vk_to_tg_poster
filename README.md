...
Если `data/config.yaml` отсутствует, контейнер создаст его автоматически с настройками по умолчанию.
...
- `data/config.yaml` — конфигурация
- `data/` — служебные данные и кэш обработанных постов
- `data/logs/` — логи

Каталог `data/` подключается как volume и сохраняется между перезапусками контейнера.
...
Для разработки и ручного запуска из исходников можно ориентироваться на `config/config.example.yaml`.
